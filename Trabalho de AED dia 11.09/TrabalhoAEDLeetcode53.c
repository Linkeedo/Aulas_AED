...
/* >>> ESCREVA SUA SOLUÇÃO AQUI DENTRO <<< */
	int soma = 0, somaTotal = 0,testeNegativo = 0,maiorNegativo = 0;
	for( int i = 0 ; i < numsSize ; i++ ){
		if ( nums [ i ] < 0 ){
			testeNegativo++;
			if ( nums [ i ] > maiorNegativo || maiorNegativo == 0 )
			maiorNegativo = nums [ i ];
		}
	}
	if( testeNegativo == numsSize)
		somaTotal = maiorNegativo;

	for ( int i = 0 ; i < numsSize ; i++){
		soma = 0;
		for( int cont = i ; cont < numsSize ; cont++){
			soma += nums [ cont ];
		if ( soma > somaTotal ){
			somaTotal = soma ;
		}
