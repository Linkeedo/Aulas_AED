...
char *multiply( char *num1, char *num2 ) {
	...
	char *resposta = malloc ( ( tamanhoTotal + 1 ) * sizeof(char) );
	do{
		resposta[i] = '0';
		i++;
	...
	for(int segundo = tamanhoSegundo - 1, casas = 0 ; segundo >= 0 ; segundo-- , casas++){
		for(int primeiro = tamanhoPrimeiro - 1, casasTotais = casas ; primeiro >= 0; primeiro-- , casasTotais++){
			...
			if(resposta [ tamanhoTotal - 1 - casasTotais ] - 48 >= 10 ){//falta só isso
				int resto = 0 , verdade = 0 ;
				if( resposta [ tamanhoTotal - 1 - casasTotais ] - 48 == 10){
					resto = (resposta [ tamanhoTotal - 1 - casasTotais ] - 48) % 10;
				}
				resto = resposta [ tamanhoTotal - 1 - casasTotais ] - 48 - 10;
				resposta [ tamanhoTotal - 1 - casasTotais ] = resto + 48;//Marca
			}
		}
	}

	while ( resposta[0] == '0' ){
		strcpy(resposta,&resposta[1]);
	}
	char *res = malloc( ( strlen( resposta ) + 1 ) * sizeof( char ) );
	res[strlen( resposta )] = '\0';
	strcpy( res, resposta );
	return res;
}
